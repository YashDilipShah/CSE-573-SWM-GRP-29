...
for i, customer_id in enumerate(customer_ids):
    ...
    nx.draw_networkx_nodes(G, pos, node_size=500, node_color='orange', alpha=0.8, ax=axs[i])  # Draw nodes
    nx.draw_networkx_edges(G, pos, width=1, alpha=0.5, edge_color='blue', ax=axs[i])  # Draw edges
    nx.draw_networkx_labels(G, pos, labels={node: node for node in G.nodes()}, font_size=12, ax=axs[i])  # Add labels
    axs[i].set_title(f"Customer ID: {customer_id}")  # Add title
